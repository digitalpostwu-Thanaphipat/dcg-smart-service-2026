...
### Phase A Local Implementation: Session Policy Alignment — 12 มิถุนายน 2569

* **สถานะ:** ดำเนินการสำเร็จใน local เท่านั้น ยังไม่ push GitHub และยังไม่ deploy Apps Script เพื่อป้องกัน Vercel auto-update ก่อนงานพร้อมขึ้น production
* **Backend version string:** อัปเดต local เป็น `2026-06-12-v35-session-policy`
* **Staff session:** หลังยืนยัน OTP สำเร็จ token ใช้ได้ถึงวันศุกร์เวลา 23:59:59.999 ของสัปดาห์เดียวกัน
* **Self-service session:** หลังยืนยัน OTP สำเร็จ token ใช้ได้ถึงสิ้นวันเดียวกันเวลา 23:59:59.999 และข้อความอีเมล OTP ปรับให้ตรงกับ policy
* **Security guard:** การเขียนข้อมูลยังต้องผ่าน `verifySessionToken` ทุกครั้ง และ `shouldRestrictWorkdays` ยังควบคุมการใช้งานเสาร์-อาทิตย์ตาม config
* **Verification:** `npm.cmd run test -- src/test/backend.test.ts` ผ่าน 15 tests, `npm.cmd run test` ผ่าน 66 tests, `npm.cmd run build` ผ่าน, `npm.cmd run lint` ผ่าน

---

### Phase B Local Implementation: Offline Session Caching and Sync Safety — 12 มิถุนายน 2569

* **สถานะ:** ดำเนินการสำเร็จใน local เท่านั้น ยังไม่ push GitHub และยังไม่ deploy Apps Script
* **Sync status model:** เพิ่มสถานะ `auth_required` และ `failed` ให้ local log/sync queue
* **Auth expired behavior:** เมื่อ token หมดอายุระหว่าง sync ระบบ mark รายการเป็น `auth_required`, เก็บ queue ไว้, แจ้งให้ยืนยัน OTP ใหม่, clear session token แต่ไม่ force logout และไม่ลบ pending data
* **Re-auth behavior:** หลัง login/OTP ใหม่ ระบบเรียก `syncPendingLogs()` เพื่อ sync รายการ `auth_required` ต่อทันที
* **Offline behavior:** ถ้า offline ระบบไม่ยิง API, เก็บ pending logs ใน IndexedDB, และนับ queue ต่อ
* **UI/report:** เพิ่ม badge `รอ OTP` และ `ซิงค์ไม่สำเร็จ` ในรายงาน รวมถึง export status text ใหม่
* **Verification:** `npm.cmd run test -- src/services/syncEngine.test.ts src/test/backend.test.ts` ผ่าน 20 tests, `npm.cmd run test` ผ่าน 71 tests, `npm.cmd run build` ผ่าน, `npm.cmd run lint` ผ่าน

---

### Phase C Local Implementation: Active Spreadsheet Rollover Safety Scaffold — 12 มิถุนายน 2569

* **สถานะ:** ดำเนินการสำเร็จเฉพาะ local dry-run/copy-only scaffold ยังไม่ push GitHub และยังไม่ deploy Apps Script
* **Backend action:** เพิ่ม `archiveRollover`
* **Dry-run:** `mode: "dry_run"` อ่านอย่างเดียว นับ rows ที่ปีงบประมาณต่ำกว่า active fiscal year จาก `Timestamp`, สรุป row count/checksum ต่อ sheet และไม่สร้าง/แก้ชีต
* **Copy-only:** `mode: "copy_only"` ต้องมี `ARCHIVE_COPY_APPROVED=true` และ `ARCHIVE_SPREADSHEET_ID` ก่อนคัดลอก rows ไป archive spreadsheet และบันทึก `Archive_Index`
* **Deletion guard:** `mode: "delete"` ถูก block ถ้าไม่มี `ARCHIVE_DELETE_APPROVED=true`; แม้มี flag แล้ว local Phase C ยังไม่ implement deletion จริงเพื่อกันลบข้อมูล active โดยไม่ผ่าน pilot
* **Transaction sheets:** รองรับ `Tx_InternalRun`, `Tx_InternalSort`, `Tx_ExternalPost`
* **Verification:** `npm.cmd run test -- src/test/backend.test.ts` ผ่าน 19 tests, `npm.cmd run test` ผ่าน 75 tests, `npm.cmd run build` ผ่าน, `npm.cmd run lint` ผ่าน

### Phase D Local Implementation: Cross-Year Read/Report Mode — 12 มิถุนายน 2569

* **สถานะ:** ดำเนินการสำเร็จใน local เท่านั้น ยังไม่ push GitHub และยังไม่ deploy Apps Script/Vercel
* **Backend routing:** เพิ่ม `searchLogsCrossYear` และ `publicSearchCrossYear` เพื่อ route ปีงบปัจจุบันไป active spreadsheet และปีงบเก่าไป archive spreadsheet ผ่าน `Archive_Index`
* **Source metadata:** response เพิ่ม `SourceFiscalYear`/`sourceFiscalYear`, `SourceType`/`sourceType`, และ spreadsheet id เพื่อให้รายงานรู้ว่า row มาจาก active หรือ archive
* **Archive read-only guard:** search archive ใช้ read path เท่านั้น ไม่สร้าง/แก้/ลบชีต และ active source ไม่ดึง rows ปีงบเก่าเมื่อ query ข้ามปีเพื่อกัน duplicate
* **Self-service event marker:** search/export/print ที่แตะ archive log เป็น `trackingMode=masked_archive`
* **Frontend mapping:** รายงานพนักงานรับ source metadata เข้า `LogItem`; self-service export/print log ตรวจ archive จากชุดข้อมูลที่ export
* **Verification:** `npm.cmd run test -- src/test/backend.test.ts` ผ่าน 21 tests, `npm.cmd run test` ผ่าน 77 tests, `npm.cmd run build` ผ่าน, `npm.cmd run lint` ผ่าน
* **ยังเหลือ:** staff Excel export ยังไม่ได้เพิ่มคอลัมน์ source โดยตรง และยังไม่ได้ทดสอบกับ archive spreadsheet จริงหลัง deploy backend

### Phase D Gap Closure: Staff Export Source Columns and Staff Archive Audit — 12 มิถุนายน 2569

* **สถานะ:** ดำเนินการสำเร็จใน local เท่านั้น ยังไม่ push GitHub และยังไม่ deploy Apps Script/Vercel
* **Staff Excel export:** เพิ่ม `Fiscal year` และ `Data source` ใน export tabs `list`, `run`, `sort`, `ext`; budget summary เพิ่ม `Archive rows`
* **Staff archive audit:** เพิ่ม backend action `logStaffReportEvent` ที่ใช้ staff session auth และบันทึกลง `Tx_SelfServiceLog` เดิมด้วย action `staff_report_search` / `staff_report_export`
* **Search logging:** `searchLogsCrossYear` log อัตโนมัติเมื่อ query แตะ archive
* **Export logging:** frontend report export log แบบ best-effort เมื่อไฟล์ export มี archive rows
* **Verification:** `npm.cmd run test -- src/test/backend.test.ts src/services/api.test.ts` ผ่าน 30 tests, `npm.cmd run test` ผ่าน 78 tests, `npm.cmd run build` ผ่าน, `npm.cmd run lint` ผ่าน
* **ยังเหลือสำหรับ Phase E:** deploy backend แล้วทดสอบ archive spreadsheet จริงตาม production release guard

### Phase E Local Implementation: Schema and Production Release Guard — 12 มิถุนายน 2569

* **สถานะ:** เริ่ม Phase E และทำ local release guard artifacts สำเร็จ ยังไม่ push GitHub และยังไม่ deploy Apps Script/Vercel
* **Release checklist:** เพิ่ม `docs/phase-e-production-release-guard.md` ครอบคลุม backend/frontend pre-deploy, post-deploy smoke test, rollback path, no-go conditions
* **Schema guard evidence:** `getSchemaAudit` คืน `mode: "read_only"` และไม่สร้าง/แก้/ลบชีตใน unit test
